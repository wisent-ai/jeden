...
- Jeden does not define the autonomous outcome-contract protocol; [Agent Contract](https://github.com/wisent-ai/agent-contract) owns its schemas, state machine, validators, prompts, and receipts, while Jeden supplies model conversations and tools through the integration interface.
...
5. **Autonomous outcome pursuit.** An operator has a rough objective but cannot stay present to correct each interpretation. They run `jeden pursue "<objective>"` with explicit execution grants. Agent Contract derives and reviews the outcome contract; Jeden supplies its Brama-backed conversations and approval-gated tools to the accepted execution stages. The result is a durable contract, verdict, receipt, and stage-session provenance under `<cwd>/.agent-contract/runs/`.
...
- **Agent Contract adapter** — `jeden pursue` maps the separately owned protocol stages onto persistent planner and executor conversations plus fresh read-only reviewers.
...
`jeden pursue` integrates the same local run loop with [Agent Contract](https://github.com/wisent-ai/agent-contract): read-only distillation, independent contract review, granted execution, and independent acceptance review. Agent Contract owns the state machine and artifacts; Jeden owns only the Brama-backed `StageRunner` adapter, its conversations, and its tool policy.
...
Use `run` when the supplied task is already the contract. Use `pursue` when the input is only an intent seed and Agent Contract must recover the concrete outcome, boundaries, preferences, evidence, and finish line before Jeden implements it; see the [Agent Contract protocol](https://github.com/wisent-ai/agent-contract#how-it-works).
...
| State | Operator-owned and local. User scope `~/.jeden/`: `sessions/`, `memory.sqlite3`, `.env`, `config.json`/`config.yml`, `cache/`, `tools/`, `extensions/`, `commands/`, `plugins/`, `hooks.json`, `mcp.json`. Project scope `<cwd>/.jeden/`: `config.json`, `usage.json`, `mode-state.json`, `subscription-cooldowns.json`, `tasks/`, `commands/`, `extensions/`, `tools/`, `hooks.json`, `mcp.json`. Agent Contract independently owns preference profiles and pursuit artifacts under `.agent-contract/`; its receipts reference Jeden session paths without moving those sessions. Session transcripts are append-only and are never expired or deleted by Jeden. |
