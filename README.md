...
- Filesystem read tools: `list_dir`, `read_file`, `read_binary_file`, `read_archive`, `read_document`, `search_text`, `search_files`, `glob_paths`, `grep_regex`. `read_file` supports selectors like `:10-30`, `:raw:10+5`, and `:conflicts`; large files require selectors so returned content stays under the cap. `read_archive` lists or reads entries from `.zip`, `.tar`, `.tar.gz`, and `.tgz`. `read_document` extracts readable text from text, HTML, JSON, notebooks, and basic PDF text streams. `list_dir` accepts `depth`/`limit` for bounded recursive listings. Recursive search and glob discovery use `git ls-files --exclude-standard` by default, so ignored files are skipped; pass `gitignore:false` and `hidden:true` to include ignored or dot paths. `search_files` and `grep_regex` accept `path` or `paths`; `grep_regex` supports cross-line matches with `multiline:true`; `search_files`, `glob_paths`, and `grep_regex` accept `limit` and `skip` for pagination.
