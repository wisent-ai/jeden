...
- Filesystem read tools: `list_dir`, `read_file`, `read_binary_file`, `search_text`, `search_files`, `glob_paths`, `grep_regex`. `list_dir` accepts `depth`/`limit` for bounded recursive listings. Recursive search and glob discovery use `git ls-files --exclude-standard` when available, so ignored files are skipped. `search_files` and `grep_regex` accept `path` or `paths`; `search_files`, `glob_paths`, and `grep_regex` accept `limit` and `skip` for pagination.
