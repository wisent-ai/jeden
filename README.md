...
- Web read tools: `fetch_url` for raw text and `fetch_readable_url` for simplified readable text. `fetch_readable_url` normalizes HTML, pretty-prints JSON, converts CSV/TSV responses to compact Markdown tables, extracts RSS/Atom item titles and links, and extracts basic PDF/notebook document text from URL responses.
