...
const commands = [
  {
    ...
    effect: "Creates or continues a local session ledger under the configured session root, calls the selected Brama model route, executes approved tools in the workspace, records usage, and prints the final answer. Ordinary user turns, including delegated tasks, must return the built-in seven-area structured report. When <code>--max-steps</code> leaves another model step, a missing or invalid report gets one corrective request; a second violation, or a first after the budget is exhausted, is refused as an error, records a rejected <code>contract_violation</code> and <code>run_error</code>, and is never printed as a successful final. <code>--model-only</code> retains its separate output format.",
    refusals: [
