    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "A Tama registry entry may name its hook by a path relative to the registry itself, such as <code>shared-hooks/block_identity_literals.py</code>. Jeden resolves such a path against the registry's own directory, preferring the absolute <code>source</code> the registry's catalog records for that hook id, so the same catalog runs from any workspace. An absolute command is run as written, and a bare program name is left to <code>PATH</code>.",
          "A registration whose file this machine does not have still refuses the tool — a guard that cannot start has judged nothing — and the refusal names it: <code>TAMA_HOOK_INFRASTRUCTURE: the hook `&lt;id&gt;` is registered in &lt;registry&gt; as `&lt;command&gt;`, and no such file exists here (tried: …)</code>. <code>/hooks</code> lists the same registrations under <em>Registrations with no executable here</em>, so a broken hook install is visible before it refuses anything.",
        ],
        commands: [
          {
