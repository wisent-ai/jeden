    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "<code>npm run test:contracts</code> compiles the real contract suite, runs the product's build/sign workflow, and executes the suite. This ordering prevents Cargo from replacing signed macOS binaries after signing. <code>target/contract-runs</code> retains source revision, binary digest, command output and final state. Model access uses the configured Brama identity and remains a real dependency.",
