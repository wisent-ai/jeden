    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Each inspection stage gets exactly one correction. When an intake or acceptance-review answer cannot be read as the required JSON object, or the native controller refuses the plan it carries, the exact refusal is recorded as <code>completion_rejected</code> and quoted back to a fresh read-only inspection. A second refusal records the durable blocker; one unusable answer no longer strands a retained request. An intake task that omits <code>kind</code> is read as work, the stricter kind, so acceptance still requires an independent observation.",
