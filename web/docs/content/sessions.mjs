    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Each inspection stage gets exactly one correction. When an intake or acceptance-review answer cannot be read as the required JSON object, or the native controller refuses the plan it carries, the exact refusal is recorded as <code>completion_rejected</code> and quoted back to a fresh read-only inspection, together with the piece of the answer the parser stopped at. A second refusal records the durable blocker; one unusable answer no longer strands a retained request. A missing field is read the strict way rather than refused: an intake task without <code>kind</code> is work, and a review without <code>criteria</code> has reviewed no criterion, so the controller still refuses the acceptance and the loop keeps the work open. A field the answer adds is ignored, and <code>taskId</code>, <code>task_id</code>, <code>task</code> and <code>id</code> are read as the same identifier, because an echoed or differently spelled name is not a different verdict. An answer the output budget cut mid-JSON is the one refusal more words cannot repair, so its correction is asked for with twice the inspection budget and with every explanation held to one sentence.",
          ...
          "The place a criterion names is decided by the controller, not by that interpretation. When a work criterion names a path, an accepted verdict requires an accepted observation at one of the places that criterion names, and a verdict anchored at none of them is refused with <code>task &lt;id&gt; criterion &lt;n&gt; names &lt;path&gt;, and no accepted observation happened there</code>. One sentence may name several places while only one of them is the requirement, as in \"the workspace root must hold alpha.txt; a copy already sits in <code>scratch/</code>\", so demanding every named place would refuse finished work over a place that was only mentioned. A bare file name is read as the workspace root the request recorded, so a write one directory below it does not satisfy the criterion. A path of several parts is matched exactly against that same workspace; only an observation that happened outside the workspace may match it by ending with those parts, because one file is spelled from different roots inside a receipt. Inside the workspace a nested copy of the same name is a different place: <code>workspace/alpha.txt</code> is not met by a write to <code>workspace/workspace/alpha.txt</code>. A criterion whose last part is not a file name is read as a place, so work done inside it counts. A slash alone is not a place: <code>input/output</code> and <code>CLI/GUI</code> are prose, and only an anchored path, a trailing slash or a segment that names a file is read as one. Version numbers, host names and gateway addresses are not paths, and a criterion that names no path is left entirely to the reviewer's prose.",
          "Every request gets a time to completion. The independent intake that records its acceptance requirements also records <code>estimate</code>: the whole minutes it expects the request to take until every task is independently verified, with the moment it was recorded, before any execution. Nothing revises that first estimate. When an independent review accepts the request, the controller records <code>completedAt</code>. A defect that reopens the request removes <code>completedAt</code> again, so the measured time runs on from the same estimate until a fresh review accepts the repair; a request closed by cancellation never gets one. An intake answer without <code>estimateMinutes</code> above zero is refused as <code>completion intake must estimate the time to completion before execution: estimateMinutes is a whole number of minutes above zero</code> and gets its one correction like any other refused intake.",
          "The completion snapshot, from <code>jeden todo list --json</code>, RPC <code>session/completion/get</code> and every <code>completion</code> event, carries <code>timing</code>: one entry per request that has an estimate, with <code>estimateMinutes</code>, <code>estimatedAt</code> and <code>dueAt</code>, and, once the request is verified complete, <code>completedAt</code>, <code>elapsedSeconds</code>, <code>differenceSeconds</code> (the actual time minus the estimate, negative when the work finished early) and <code>ratio</code>. <code>state</code> is <code>open</code>, <code>on_time</code>, <code>late</code> or <code>cancelled</code>; every <code>*At</code> is Unix seconds. <code>jeden todo list</code> prints the same comparison under each request, for example <code>Time to completion: estimated 5 min · done in 1 min 45 s, 3 min 15 s under the estimate</code>, and the Tasks panels of Jeden Desktop and Jeden iOS print the same words. The terminal shows the estimate as soon as the intake records it, and the answer of the turn in which a request is accepted ends with its measured time in the conversation language. Completion files written before schema version 3 open unchanged and are carried forward; a file written by a newer Jeden is refused as <code>unsupported completion state version: &lt;n&gt;</code>.",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Version 2 of the contract adds the time to completion: before implementation the task gets an estimate and the user is told when it will be done, that first estimate is never changed to fit the result, and the actual time is reported beside it. Jeden records the estimate at intake and measures the time to verified completion itself, so the time is not a report entry and the model cannot flatter it. <code>jeden contracts install --omp</code> carries the same clause into Omp, where no harness measures it and the agent states both figures itself.",
