    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "The place a criterion names is decided by the controller, not by that interpretation. When a work criterion names a path, an accepted verdict requires an accepted observation at that path, and a verdict resting on work done elsewhere is refused with <code>task &lt;id&gt; criterion &lt;n&gt; names &lt;path&gt;, and no accepted observation happened there</code>. A bare file name is read as the workspace root the request recorded, so a write one directory below it no longer satisfies the criterion. A path of several parts is matched exactly against that same workspace; only an observation that happened outside the workspace may match it by ending with those parts, because one file is spelled from different roots inside a receipt. Inside the workspace a nested copy of the same name is a different place: <code>workspace/alpha.txt</code> is not met by a write to <code>workspace/workspace/alpha.txt</code>. A criterion whose last part is not a file name is read as a place, so work done inside it counts. Version numbers, host names and gateway addresses are not read as paths, and a criterion that names no path is left entirely to the reviewer's prose.",
