    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Waiting is bounded in three separate places, because a gateway that is down, a model still reading a long prompt, and a stream that stopped mid-answer are three different failures. A reasoning route may take minutes to send its first token, so a turn waits five minutes for it before reporting <code>model stream first-event timeout</code>; a stream that has produced output and then goes quiet is cut by <code>modelRouting.retry.idleTimeoutMs</code>; and an unreachable gateway fails at once as a transport error rather than waiting at all. Writing <code>modelRouting.retry.firstEventTimeoutMs</code> into <code>.jeden/config.json</code> replaces the five minutes with exactly that value.",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Release builders receive private Git dependencies through the immutable <code>private-cargo-sources</code> input, not through GitHub credentials or sibling checkouts. After a <code>Cargo.lock</code> change, run <code>python3 release/cargo.py export .wisent-output/private-cargo-sources.tar.gz</code>, publish the returned archive with <code>stado storage put &lt;input.uri&gt; &lt;archive&gt; --if-absent</code>, and record its returned input object in <code>.wisent-release.json</code>. Python 3.9 or newer is required. The release Cargo wrapper refuses missing inputs and inputs from another lockfile; Cargo verifies the exported checksums without changing the lockfile.",
          "Both release steps let Stado resolve Python directly: quality runs <code>python3 release/cargo.py cargo ...</code> and building runs <code>python3 release/cargo.py stage --bin jeden</code>, adding <code>--bin jeden-sandbox-helper</code> on Darwin. No shell selects a second interpreter. Staging prints the actual interpreter and copies only successful native build outputs from the source's <code>target</code> directory into <code>WISENT_OUTPUT_DIR/bin</code>. An absent output directory setting is refused with <code>WISENT_OUTPUT_DIR is required for native staging</code> before compilation.",
          "The wrapper finds Cargo on <code>PATH</code>, then in <code>$CARGO_HOME/bin</code>, defaulting to <code>$HOME/.cargo/bin</code>. It preserves the proxy name used by Rustup and does not require shell startup files. If no executable exists, the refusal names the missing path and asks for toolchain provisioning; the wrapper does not install tools or change host configuration.",
          "Run <code>python3 tests/release/private-sources.py</code> with Rust and Python 3.12 to exercise the export, offline source selection with a service-style <code>PATH</code>, actual helper staging and execution, and missing-output, missing-toolchain, missing-input, lockfile-mismatch and missing-package refusals. Reports under <code>.wisent-output/release-tests/</code> retain the source revision, patch, command output, exit codes and staged helper hash. This check does not qualify a signed sandbox or a model-backed task.",
          "Each native release also runs <code>python3 tests/release/private-sources.py --stage</code> using its declared input. This mode builds and executes the staged Jeden CLI and checks staging refusals without exporting private sources or needing GitHub credentials. It records the worker's <code>WISENT_SOURCE_COMMIT</code>; the archive retains its reports under <code>evidence/release-tests/</code>.",
        ],
        commands: [
          {
