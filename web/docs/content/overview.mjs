    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Release builders receive private Git dependencies through the immutable <code>private-cargo-sources</code> input, not through GitHub credentials or sibling checkouts. After a <code>Cargo.lock</code> change, run <code>cargo run --locked --manifest-path tools/Cargo.toml -- release export .wisent-output/private-cargo-sources.tar.gz</code>, publish the returned archive with <code>stado storage put &lt;input.uri&gt; &lt;archive&gt; --if-absent</code>, and record its returned input object in <code>.wisent-release.json</code>. The export refuses an archive path outside <code>.wisent-output</code> and a lockfile that changes during the export. The release Cargo wrapper refuses missing inputs and inputs from another lockfile; Cargo verifies the exported checksums without changing the lockfile.",
          "The release tooling is the Rust package in <code>tools/</code>, its own Cargo workspace so it builds before the private sources are configured. Quality runs <code>cargo run --locked --manifest-path tools/Cargo.toml -- release cargo ...</code> and building runs <code>... -- release stage --bin jeden</code>, adding <code>--bin jeden-sandbox-helper</code>. Staging names the tool it runs as and copies only successful native build outputs from the source's <code>target</code> directory into <code>WISENT_OUTPUT_DIR/bin</code>. An absent output directory setting is refused with <code>WISENT_OUTPUT_DIR is required for native staging</code> before compilation.",
