...
export const completionCommands = [
  ...
  },
  {
    ...
    inputs: [selection],
    effect: "Prints status, revision, original requests, acceptance criteria, failed operations and verified session/event references. A task that waits on you shows <code>Waiting on you: &lt;ask&gt;</code> and the exact <code>jeden todo answer</code> command, and the status reads <code>waiting_for_operator</code> until you answer; an answered ask stays beside the task as <code>Asked of you</code> and <code>Your answer</code>. <code>complete: false</code> remains explicit when there is unplanned, pending, paused, interrupted, blocked, waiting or unverified work. Under each request it prints the time to completion: the estimate the independent intake recorded before execution beside the time so far, or, once an independent review accepted the request, the time it took and the difference, as in <code>Time to completion: estimated 5 min · done in 1 min 45 s, 3 min 15 s under the estimate</code>. A request not planned yet reads <code>Time to completion: not estimated yet; Jeden records the estimate before execution</code>, and one closed without an estimate reads <code>Time to completion: no estimate was recorded</code>. <code>--json</code> carries the same figures in <code>timing</code>. Reading a legacy workspace list migrates that superseded list before displaying its session-owned replacement.",
    refusals: [failures[0], "Invalid or unreadable completion files return their storage or schema error, never an empty successful task list; a file written by a newer Jeden is refused as <code>unsupported completion state version: &lt;n&gt;</code>."],
  },
  {
    path: "todo/add",
