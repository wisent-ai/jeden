export const sessionCommands = [
  ...
  },
  {
    ...
    inputs: [
      ...
      "Real admission and persistence journeys run with <code>python3 -m unittest discover -s tests/pursuit -p test_requests.py -v</code>. They invoke the actual CLI, refuse ungranted writes, retain a blocked non-checkout request, reopen it and reject changed input under the same identity. Reports under <code>target/pursuit-runs</code> retain executable hashes, source identity, commands and exit statuses. Fleet builds use their staged executable and retain the reports in <code>evidence/pursuit-tests</code>; missing candidate source provenance fails qualification. These refusal journeys do not qualify successful model execution, interrupted external effects or delivery.",
    ],
