export const sessionCommands = [
  {
    ...
    inputs: [
      "Required: one or more paths. A path is a transcript file another harness wrote, or a directory that is scanned recursively for such files. Which harness wrote a file is read from its content, never from the command: today the supported format is <code>omp</code>, a JSONL transcript whose <code>{\"type\":\"session\"}</code> header carries the session id and workspace. The workspace and title come from the transcript itself.",
      "The destination is the normal Jeden session root, or JEDEN_SESSION_ROOT when explicitly set. <code>--refresh</code> replaces only a never-adopted native import; it never rewrites an already-used native conversation.",
      "Jeden Desktop exposes File → Import Sessions. Enter absolute paths on the Stado-selected host, one per line. The window reports each imported path and each refusal; it does not start agents or transfer execution ownership.",
    ],
    effect: "Returns JSON with imported, existing, sessions and failures. Each successful row names format, sourceSession, sourcePath, sessionPath, cwd, title and interrupted. The native session is keyed by the format and the source id (<code>omp-&lt;sha256&gt;</code>), so importing the same source again reports it as existing and refreshing it replaces the same directory. The full source file is retained at artifacts/&lt;format&gt;-source.jsonl. The active source branch becomes a native context snapshot; unanswered requests enter the native retained-work store. Import does not stop the other harness, start a model, or transfer execution ownership. A copied history is not a completed migration. Inspect failures even when the command exits successfully: per-source failures do not discard other successful imports.",
    refusals: [
      "No path is refused as <code>Usage: jeden import &lt;path&gt;... [--refresh] [--json]</code>. A file named outright that is no supported transcript is a failure row: <code>&lt;path&gt;: not a transcript of a supported harness (supported: omp)</code>; inside a scanned directory such files are skipped without a word. Paths that yield nothing at all are refused as <code>no sessions to import under &lt;paths&gt;</code>.",
      "Unreadable sources, malformed JSON records, missing ancestors, cyclic ancestry and an unavailable source workspace are returned per source. A source changed during import is refused, leaving its original history intact. A changed source from an earlier import is refused as <code>source changed since import; use --refresh only for a never-adopted import</code>. Adopted imports, child sessions and changed native retained work refuse refresh.",
      "A concurrent import refuses its import lock. Interrupted replacement publication is recovered before the next import attempts that source.",
    ],
  },
  {
    path: "run",
    ...
    refusals: [
      ...
      "After a request is captured, a preparation failure is retained in the session transcript as <code>run_error</code>, with <code>operation: prepare_turn</code> and the original failure message. Inspect it with <code>jeden show &lt;session-id&gt;</code>. The same conversation path serves CLI and RPC; a recorded refusal is not successful execution and does not complete the retained request. Failures before conversation creation are not covered by this event. If writing the event also fails, the returned error names both failures.",
  ...
  },
  {
    ...
    inputs: [
      ...
      "<code>session/completion/get</code> takes <code>{sessionId}</code>; <code>session/completion/control</code> takes <code>{sessionId,taskId,action,reason,revision}</code>, with action pause, resume, cancel, defect or answer. For answer, <code>reason</code> carries the operator's answer to the task's recorded ask; a task that asked nothing refuses it. Both return <code>{sessionId,completion}</code>, whose tasks carry <code>operatorRequest {askedAt,ask,answer?}</code> when a review asked the operator for something and whose status reads <code>waiting_for_operator</code> while an ask is unanswered. Task IDs may also name an original request.",
      ...
      "session/import takes {paths,refresh?} — the same paths the CLI takes, as an array of strings — and returns the same report as <code>jeden import</code> without invoking a model or terminating a source process. A missing or empty paths array is refused as invalid_params.",
    ],
