export const sessionCommands = [
  ...
  },
  {
    ...
    inputs: [
      ...
      "Real admission and persistence journeys run with <code>cargo test --test pursuit -- --ignored</code> against a candidate named by <code>JEDEN_TEST_BINARY</code> (else <code>$WISENT_OUTPUT_DIR/bin/jeden</code>, else the binary the test build produced) and bound to <code>WISENT_SOURCE_COMMIT</code>. They invoke the actual CLI, refuse ungranted writes, retain a blocked non-checkout request, reopen it and reject changed input under the same identity. Reports under <code>target/pursuit-runs</code> retain executable hashes, source identity, commands and exit statuses. Fleet builds run them through the release Cargo wrapper against their staged executable and retain the reports in <code>evidence/pursuit-tests</code>; missing candidate source provenance fails qualification. These refusal journeys do not qualify successful model execution, interrupted external effects or delivery.",
    ],
