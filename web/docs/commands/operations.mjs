export const operationCommands = [
  ...
  },
  {
    ...
    refusals: [
      ...
      "Signature verification and the helper enforcement probe each run to completion: the verifier's own verdict is the answer, and a slow machine is never reported as an invalid signature. A probe that fails reports the failed operation and the helper path, and the sandbox stays unavailable; neither agent execution nor a successful standalone helper probe overrides missing verification.",
