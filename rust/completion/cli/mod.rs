...
pub(crate) fn workspace(session: &Path) -> Result<PathBuf, String> {
    ...
    state
        ...
        .map(PathBuf::from)
        .ok_or_else(|| "session has no recorded workspace".into())
}
...
pub(crate) fn active_session(cwd: &Path, selected: Option<&str>) -> Result<PathBuf, String> {
    let path = match selected {
        ...
        None => crate::slash::read_mode_state(cwd)
            .last_session_path
...
pub(crate) fn render(state: &CompletionState) -> String {
    ...
    for request in &state.requests {
        lines.push(format!(
            ...
            if request.paused {
                "paused"
            } else if request.coverage_verified {
                "covered"
            } else if request.planned {
                "planned"
            } else {
                "unplanned"
            },
            ...
            .filter(|task| task.request_id == request.id)
        {
            ...
            if let Some(verification) = &task.verification {
                for evidence in &verification.evidence {
                    lines.push(format!(
                        "    Evidence: {}#{}",
                        evidence.session_path, evidence.event_id
                    ));
                }
            }
        }
...
pub(crate) fn execute(
    ...
    let (action, remaining) = words
        .split_first()
        .map(|(action, rest)| (*action, rest))
        .unwrap_or(("list", &[]));
    if !matches!(
        action,
    ...
    let session = if action == "add"
        && selected.is_none()
        && crate::slash::read_mode_state(cwd)
            .last_session_path
            .is_none()
    {
    ...
    let state = match action {
        ...
        "continue" if remaining.is_empty() => {
            let mut args = run_args
                .cloned()
                .ok_or("Use jeden todo continue with the current execution grants.")?;
