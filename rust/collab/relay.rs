...
mod tests {
    use super::*;
    use crate::collab::{
        new_role_write_token, open_frame, seal_frame, CollabRole, FrameKind, ProtocolFrame,
    };
    use std::io::{Read, Write};
    ...
    fn collab_runtime_three_role_tokens_authorize_only_their_provisioned_role_and_mismatch_is_denied(
        ...
        assert_eq!(
            store
                .post_authorized("room", "full-event".into(), Some(&full))
                .unwrap(),
            Some(1)
        );
        ...
        assert_eq!(
            store
                .post_authorized("room", "prompt-event".into(), Some(&prompt))
                .unwrap(),
            Some(2)
        );
        assert_eq!(
            store
                .post_authorized("room", "abort-event".into(), Some(&abort))
                .unwrap(),
            Some(3)
        );
        ...
        assert!(
            response.contains("write token is not valid for requested role"),
            "{response}"
        );
        assert_eq!(
            store.get("room", 0).0,
            vec!["full-event", "prompt-event", "abort-event"]
        );

        assert!(store
            .post_authorized(
                "room",
                "wrong-room-role".into(),
                Some(&new_role_write_token(CollabRole::Prompt))
            )
            .is_err());
    }
    ...
    fn collab_runtime_view_replays_but_cannot_write_or_rotate_tokens() {
        ...
        assert_eq!(
            store
                .post_authorized("view-room", "seed-event".into(), Some(&full))
                .unwrap(),
            Some(1)
        );
    ...
    fn collab_runtime_e2ee_relay_persists_no_plaintext_or_key_material() {
        ...
        assert_eq!(
            store
                .post_authorized("secure-room", blob, Some(&token))
                .unwrap(),
            Some(1)
        );

        let conn = Connection::open(&fixture.db).unwrap();
        let persisted_blob: String = conn
            .query_row(
                "SELECT blob FROM events WHERE room_id='secure-room'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        let persisted_token_hash: String = conn
            .query_row(
                "SELECT token_hash FROM room_tokens WHERE room_id='secure-room'",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    fn collab_runtime_typed_replay_is_ordered_and_restart_cursor_resumes_after_last_seen_event() {
        ...
        for (index, frame) in frames.iter().enumerate() {
            let seq = store
                .post_authorized("replay-room", seal_frame(&key, frame).unwrap(), Some(&full))
                .unwrap();
        ...
        assert_eq!(cursor, 3);
        let initial = initial_blobs
            .iter()
            .map(|blob| open_frame(&key, blob).unwrap())
            .collect::<Vec<_>>();
        ...
        let fourth = ProtocolFrame::new(
            "host",
            CollabRole::Full,
            FrameKind::State {
                value: serde_json::json!({"step": 4}),
            },
        )
        .unwrap();
        assert_eq!(
