...
mod tests {
    ...
    #[test]
    fn executor_applies_backpressure_then_drains_accepted_work() {
        ...
        executor.drain(Duration::from_secs(5)).unwrap();
        assert_eq!(completed.load(Ordering::SeqCst), 2);
        assert_eq!(executor.readiness(), Readiness::Stopped);
        assert_eq!(executor.submit(|| {}), Err(SubmitError::NotReady));
    }
}
