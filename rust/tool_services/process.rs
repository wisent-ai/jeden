...
pub(crate) fn run(
    ...
    command.stdin = stdin;
    let result = ProcessManager.run(context, command, timeout).map_err(|detail| ServiceError::Backend { service, detail })?;
    match result.reason {
    ...
    if !result.status.success() {
        let detail = if result.stderr.text.trim().is_empty() { result.stdout.text } else { result.stderr.text };
        return Err(ServiceError::Backend { service, detail: detail.trim().to_string() });
    }
...
pub(crate) fn run_json(
    ...
    serde_json::from_str(&output).map_err(|error| ServiceError::Protocol { service, detail: format!("invalid JSON response: {error}") })
}
