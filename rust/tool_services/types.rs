...
impl HealthDescriptor {
    pub fn healthy(service: &'static str, backend: impl Into<String>) -> Self {
        Self { service, status: HealthStatus::Healthy, backend: Some(backend.into()), detail: "configured and discoverable".into() }
    }
    pub fn degraded(service: &'static str, backend: impl Into<String>, detail: impl Into<String>) -> Self {
        Self { service, status: HealthStatus::Degraded, backend: Some(backend.into()), detail: detail.into() }
    }
    pub fn unavailable(service: &'static str, detail: impl Into<String>) -> Self {
        Self { service, status: HealthStatus::Unavailable, backend: None, detail: detail.into() }
    }
    pub fn available(&self) -> bool { self.status != HealthStatus::Unavailable }
}
...
impl From<std::io::Error> for ServiceError { fn from(value: std::io::Error) -> Self { Self::Io(value.to_string()) } }
...
pub fn check_operation(context: &OperationContext<'_>) -> ServiceResult<()> {
    ...
    if context.deadline().is_some_and(|deadline| std::time::Instant::now() >= deadline) { return Err(ServiceError::DeadlineExceeded); }
    Ok(())
}

pub fn bounded_json(context: &OperationContext<'_>, service: &'static str, value: &Value) -> ServiceResult<Value> {
    check_operation(context)?;
    let bytes = serde_json::to_vec(value).map_err(|e| ServiceError::Protocol { service, detail: e.to_string() })?;
    let mut output = BoundedOutput::new(service, context.output_limits(), context.artifacts().clone());
    output.write_chunk(&bytes)?;
    context.progress(OperationProgress { stream: service, bytes: bytes.len() as u64, total_bytes: bytes.len() as u64 });
    let capture = output.finish()?;
    if !capture.truncated { return Ok(value.clone()); }
    Ok(json!({
        "ok": true,
        "truncated": true,
...
pub fn write_media_artifact(context: &OperationContext<'_>, service: &'static str, extension: &str, bytes: &[u8]) -> ServiceResult<Value> {
    check_operation(context)?;
    if bytes.len() > MAX_MEDIA_BYTES { return Err(ServiceError::OutputLimit { limit: MAX_MEDIA_BYTES }); }
    let extension = extension.trim_start_matches('.');
    if extension.is_empty() || !extension.bytes().all(|b| b.is_ascii_alphanumeric()) { return Err(ServiceError::InvalidInput("invalid artifact extension".into())); }
    fs::create_dir_all(context.artifacts().root())?;
    let path = loop {
        let id = ARTIFACT_ID.fetch_add(1, Ordering::Relaxed);
        let candidate = context.artifacts().root().join(format!("{service}-{}-{id}.{extension}", std::process::id()));
        match OpenOptions::new().create_new(true).write(true).open(&candidate) {
            Ok(mut file) => { file.write_all(bytes)?; file.sync_all()?; break candidate; }
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => continue,
            Err(error) => return Err(error.into()),
        }
    };
    context.progress(OperationProgress { stream: service, bytes: bytes.len() as u64, total_bytes: bytes.len() as u64 });
    Ok(json!({ "artifact": path.display().to_string(), "bytes": bytes.len(), "sha256": hex::encode(Sha256::digest(bytes)) }))
}
...
pub fn command_exists(program: &str) -> bool {
    ...
    std::env::var_os("PATH").into_iter().flat_map(|paths| std::env::split_paths(&paths).collect::<Vec<_>>())
        .any(|directory| directory.join(program).is_file())
}

pub fn config_path(cwd: &Path) -> Vec<PathBuf> {
    let mut paths = Vec::new();
    if let Some(home) = std::env::var_os("HOME") { paths.push(PathBuf::from(home).join(".jeden/config.json")); }
