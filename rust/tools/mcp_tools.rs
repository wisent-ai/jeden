...
pub fn native_mcp_tool_target(cwd: &Path, native_name: &str) -> Option<(String, String)> {
    crate::mcp::live_tools(cwd, 30_000).ok()?.into_iter().find_map(|(server_name, tool)| {
        let raw_name = tool.get("name").and_then(Value::as_str)?;
        (native_mcp_tool_name(&server_name, raw_name) == native_name).then(|| (server_name, raw_name.to_string()))
    })
}
