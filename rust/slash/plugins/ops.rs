...
use super::fetch::{catalog_find_plugin, copy_dir_recursive, fetch_marketplace, materialize_plugin, plugin_manifest_version, read_marketplace_catalog};
...
pub(crate) fn merged_installed_values(cwd: &Path) -> Vec<Value> {
    ...
    if let Some(installed) = plugin_registry(&plugins_home()).get("installed").and_then(Value::as_object) {
        for (id, entry) in installed { map.insert(id.clone(), entry.clone()); }
    }
    if let Some(installed) = plugin_registry(cwd).get("installed").and_then(Value::as_object) {
...
pub(crate) fn install_one(cwd: &Path, mkt_name: &str, plugin_name: &str, scope: &str, force: bool) -> Result<String, String> {
    ...
    let final_dir = plugin_cache_root().join(format!(
        "{}___{}___{}", mkt_name, plugin_name, sanitize_marketplace_name(&version)));
    if final_dir.exists() {
    ...
    let plural = match command_files.as_slice() { [_] => "", _ => "s" };
    Ok(format!(
