...
pub(crate) fn resolve_relative_plugin_path(
    mkt_cache: &Path,
    plugin_root: &str,
    source: &str,
    ...
    let stripped = rel
        .strip_prefix("./")
        .ok_or_else(|| format!("relative plugin source must start with ./: {rel}"))?;
    if stripped.is_empty() {
        return Err("empty relative plugin source".into());
    }
    ...
    if !root.is_empty() {
        for comp in root.split('/') {
            if comp.is_empty() || comp == "." || comp == ".." {
                return Err(format!("invalid pluginRoot: {plugin_root}"));
            }
            path.push(comp);
        }
    }
...
pub(crate) fn plugin_manifest_version(plugin_dir: &Path) -> Option<String> {
    read_json_value(&plugin_dir.join("package.json"))
        .get("version")
        .and_then(Value::as_str)
        .map(str::to_string)
        .or_else(|| {
            read_json_value(&plugin_dir.join(".claude-plugin/plugin.json"))
                .get("version")
                .and_then(Value::as_str)
                .map(str::to_string)
        })
    ...
    let result = (|| -> Result<Materialized, String> {
        match &source {
            Value::String(rel) => {
                ...
                if !src.is_dir() {
                    return Err(format!(
                        "plugin path not found in marketplace: {}",
                        src.display()
                    ));
                }
                ...
                match kind {
                    "github" => {
                        let repo = map
                            .get("repo")
                            .and_then(Value::as_str)
                            .ok_or("github plugin source missing repo")?;
                        ...
                        Ok(Materialized {
                            staging: staging.clone(),
                            source_desc: format!("github:{repo}"),
                            sha,
                        })
                    }
                    "url" => {
                        let url = map
                            .get("url")
                            .and_then(Value::as_str)
                            .ok_or("url plugin source missing url")?;
                        let git_ref = map.get("ref").and_then(Value::as_str);
                        git_clone(url, git_ref, &staging)?;
                        let sha = run_git(&["rev-parse", "HEAD"], Some(&staging)).ok();
                        let _ = fs::remove_dir_all(staging.join(".git"));
                    ...
                    "git-subdir" => {
                        let url = map
                            .get("url")
                            .and_then(Value::as_str)
                            .ok_or("git-subdir plugin source missing url")?;
                        let subpath = map
                            .get("path")
                            .and_then(Value::as_str)
                            .ok_or("git-subdir plugin source missing path")?;
                        let git_ref = map.get("ref").and_then(Value::as_str);
                        ...
                        let normalized = format!(
                            "./{}",
                            subpath.trim_start_matches("./").trim_start_matches('/')
                        );
                        ...
                        if !sub.is_dir() {
                            let _ = fs::remove_dir_all(&tmp);
                            return Err(format!("git-subdir path not found: {subpath}"));
                        }
                        copy_dir_recursive(&sub, &staging)?;
                        let _ = fs::remove_dir_all(&tmp);
                        Ok(Materialized {
                            staging: staging.clone(),
                            source_desc: format!("git-subdir:{url}#{subpath}"),
                            sha,
                        })
                    }
