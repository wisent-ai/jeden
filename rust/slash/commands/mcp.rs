...
fn mcp_write_file(path: &Path, mut config: Value) -> Result<PathBuf, String> {
    ...
    obj.entry("$schema").or_insert_with(|| json!("https://raw.githubusercontent.com/can1357/oh-my-pi/main/packages/coding-agent/src/config/mcp-schema.json"));
...
fn active_mcp_server_names(cwd: &Path) -> Vec<String> {
    ...
    let disabled = config.get("disabledServers").and_then(Value::as_array).into_iter().flatten().filter_map(Value::as_str).collect::<std::collections::BTreeSet<_>>();
    let mut names = config.get("mcpServers").and_then(Value::as_object)
        .map(|servers| servers.keys().filter(|name| !disabled.contains(name.as_str())).cloned().collect::<Vec<_>>())
        .unwrap_or_default();
    names.sort();
    names
}
...
fn format_mcp_list(cwd: &Path) -> String {
    ...
    let disabled = merged.get("disabledServers").and_then(Value::as_array).into_iter().flatten().filter_map(Value::as_str).collect::<std::collections::BTreeSet<_>>();
    let Some(servers) = merged.get("mcpServers").and_then(Value::as_object) else { return "No MCP servers configured.".into(); };
    if servers.is_empty() {
        return "No MCP servers configured.".into();
    }
    let mut lines = Vec::new();
    for (name, server) in std::collections::BTreeMap::from_iter(servers.iter()) {
        let source = if project.get("mcpServers").and_then(Value::as_object).map(|m| m.contains_key(name)).unwrap_or(false) {
            "project"
        } else if user.get("mcpServers").and_then(Value::as_object).map(|m| m.contains_key(name)).unwrap_or(false) {
            "user"
        } else {
            "merged"
        };
        let status = if disabled.contains(name.as_str()) || server.get("enabled").and_then(Value::as_bool) == Some(false) { "disabled" } else { "enabled" };
        let transport = server.get("type").and_then(Value::as_str).unwrap_or(if server.get("url").is_some() { "http" } else { "stdio" });
...
pub(crate) fn handle_mcp(args: &str, context: &SlashContext<'_>) -> Result<String, String> {
    ...
    match verb {
        "list" | "status" => Ok(format_mcp_list(context.cwd)),
        "add" => {
            let (_force, scope, parts) = parse_marketplace_flags(&split_args(rest));
            let scope = normalize_scope(scope)?;
            let (name, after_name) = head_rest(&parts);
            if !valid_mcp_server_name(name) { return Err("Usage: /mcp add [--scope user|project] <name> [stdio] <command> [args...]".into()); }
            let (transport_or_command, after_transport) = head_rest(after_name);
            if transport_or_command.is_empty() { return Err("Usage: /mcp add [--scope user|project] <name> [stdio] <command> [args...]".into()); }
            let server = match transport_or_command {
                "http" | "sse" => return Err(format!("MCP {transport_or_command} transport is not supported yet; use stdio.")),
                "stdio" => {
        ...
        "reload" => {
            ...
            let names = active_mcp_server_names(context.cwd);
            if names.is_empty() {
                return Ok("No MCP servers configured.".into());
            }
            let mut lines = vec![format!("Reloaded MCP config: {} server(s).", names.len())];
            for name in &names {
                match mcp::list_tools(context.cwd, name, u64::MAX) {
                    Ok(result) => {
                        let count = result.get("tools").and_then(Value::as_array).map(|t| t.len()).unwrap_or_default();
        ...
        "reconnect" => {
            ...
            match mcp::list_tools(context.cwd, server, u64::MAX) {
                Ok(result) => {
                    let count = result.get("tools").and_then(Value::as_array).map(|t| t.len()).unwrap_or_default();
