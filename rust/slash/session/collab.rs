...
fn save_collab_state(cwd: &Path, state: &Value) -> Result<PathBuf, String> {
    ...
    write_json_value(&file, &json!({ "updatedAt": now_text(), "host": host, "guest": guest }))?;
    Ok(file)
}
...
fn post_collab_http(base: &str, room: &str, key: &[u8], event_type: &str, cwd: &Path) -> Result<(), String> {
    let event = json!({ "ts": now_text(), "type": event_type, "cwd": cwd });
    let plain = serde_json::to_vec(&event).map_err(|e| e.to_string())?;
    let key_array = key.try_into().map_err(|_| "collab key has an unexpected length".to_string())?;
    let blob = crate::collab::encrypt_blob(key_array, &plain)?;
...
fn collab_http_role_status(role: &str, entry: &Value) -> String {
    ...
    let count = crate::collab::relay_get(base, room, usize::default()).map(|(events, _)| events.len());
...
pub(crate) fn handle_join(args: &str, context: &SlashContext<'_>) -> Result<String, String> {
    ...
    if target.starts_with("http://") || target.starts_with("https://") {
        ...
        let (blobs, _) = crate::collab::relay_get(&parsed.base, &parsed.room, usize::default())?;
        if blobs.is_empty() {
        ...
        // Every blob decrypted (the loop bails otherwise), so the count is the
        // number of blobs — no running counter literal needed.
        let decrypted = blobs.len();
