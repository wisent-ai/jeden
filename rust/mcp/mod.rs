...
pub fn load_config(cwd: &Path) -> Value {
    ...
    for source in [&user, &project] {
        if let Some(values) = source.get("disabledServers").and_then(Value::as_array) {
            disabled.extend(values.iter().filter_map(Value::as_str).map(ToOwned::to_owned));
        }
    }
...
fn configured_servers(cwd: &Path) -> Result<BTreeMap<String, Value>, String> {
    ...
    let servers = config.get("mcpServers").and_then(Value::as_object).ok_or("mcpServers must be an object")?;
    Ok(servers.iter().filter(|(name, _)| !disabled.contains(name.as_str())).map(|(name, server)| (name.clone(), server.clone())).collect())
}
...
fn configured_server(cwd: &Path, server_name: &str) -> Result<Value, String> {
    ...
    config.get("mcpServers").and_then(Value::as_object).and_then(|servers| servers.get(server_name)).cloned().ok_or_else(|| format!("unknown MCP server: {server_name}"))
}
...
impl ServerConnection {
    ...
    fn connect(&mut self, cwd: &Path, timeout_ms: u64, force: bool) -> Result<(), String> {
        ...
        if !force {
            if let Some(retry_after) = self.retry_after {
                if retry_after > Instant::now() && self.failures >= CIRCUIT_FAILURE_LIMIT {
                    self.state = ConnectionState::CircuitOpen;
                    return Err(format!("MCP circuit is open for {}ms", retry_after.duration_since(Instant::now()).as_millis()));
                }
                if retry_after > Instant::now() {
                    thread::sleep(retry_after.duration_since(Instant::now()).min(Duration::from_secs(2)));
                }
            }
        }
        ...
        let result: Result<(), String> = (|| {
            ...
            let capabilities = initialize.get("capabilities").and_then(Value::as_object).ok_or("MCP initialize capabilities must be an object")?;
    ...
    fn request(&mut self, cwd: &Path, method: &str, params: Value, timeout_ms: u64) -> Result<Value, String> {
        self.connect(cwd, timeout_ms, false)?;
        let first = self.client.as_mut().ok_or("MCP connection unavailable")?.request(method, params.clone(), timeout_ms);
        let result = match first {
            Ok(value) => Ok(value),
            Err(error) => {
                let dead = self.client.as_mut().map(|client| !client.is_alive()).unwrap_or(true);
                let transport_error = dead || error.contains("transport") || error.contains("stdio") || error.contains("exceeded");
                if !transport_error {
                    return Err(error);
                }
                self.record_failure(error);
                self.connect(cwd, timeout_ms, false)?;
                let retry = self.client.as_mut().ok_or("MCP connection unavailable")?.request(method, params, timeout_ms);
                if let Err(error) = &retry {
                    let dead = self.client.as_mut().map(|client| !client.is_alive()).unwrap_or(true);
                    if dead || error.contains("transport") || error.contains("stdio") || error.contains("exceeded") {
                        self.record_failure(error.clone());
                    }
                }
                retry
        ...
        let notifications = self.client.as_mut().map(McpClient::take_notifications).unwrap_or_default();
        self.process_notifications(cwd, notifications, timeout_ms)?;
        Ok(result)
    }

    fn process_notifications(&mut self, _cwd: &Path, notifications: Vec<Value>, timeout_ms: u64) -> Result<(), String> {
        ...
        for notification in notifications {
            match notification.get("method").and_then(Value::as_str) {
                ...
                Some("notifications/message") | Some("notifications/progress") | Some("notifications/resources/updated") | Some("notifications/cancelled") => {}
                Some(method) => return Err(format!("unsupported MCP notification method: {method}")),
    ...
    fn sync_config(&mut self, cwd: &Path) -> Result<(), String> {
        ...
        for (name, config) in configured {
            match self.servers.get_mut(&name) {
                ...
                Some(_) => {}
                None => { self.servers.insert(name, ServerConnection::new(config)); }
            }
        }
        Ok(())
    }
...
static MANAGERS: LazyLock<Mutex<BTreeMap<PathBuf, McpManager>>> = LazyLock::new(|| Mutex::new(BTreeMap::new()));
...
fn managers() -> Result<MutexGuard<'static, BTreeMap<PathBuf, McpManager>>, String> {
    MANAGERS.lock().map_err(|_| "MCP manager lock is poisoned".to_string())
}

fn with_connection<T>(cwd: &Path, server_name: &str, operation: impl FnOnce(&mut ServerConnection) -> Result<T, String>) -> Result<T, String> {
    let key = session_key(cwd);
    let mut managers = managers()?;
    let manager = managers.entry(key).or_default();
    manager.sync_config(cwd)?;
    let connection = manager.servers.get_mut(server_name).ok_or_else(|| format!("unknown or disabled MCP server: {server_name}"))?;
    operation(connection)
}

fn validate_tools(value: &Value) -> Result<(), String> {
    let tools = value.get("tools").and_then(Value::as_array).ok_or("MCP tools/list result must contain a tools array")?;
    for tool in tools {
        if tool.get("name").and_then(Value::as_str).filter(|name| !name.is_empty()).is_none() {
...
fn validate_resources(value: &Value) -> Result<(), String> {
    let resources = value.get("resources").and_then(Value::as_array).ok_or("MCP resources/list result must contain a resources array")?;
    for resource in resources {
        if resource.get("uri").and_then(Value::as_str).filter(|uri| !uri.is_empty()).is_none() || resource.get("name").and_then(Value::as_str).filter(|name| !name.is_empty()).is_none() {
...
fn validate_prompts(value: &Value) -> Result<(), String> {
    let prompts = value.get("prompts").and_then(Value::as_array).ok_or("MCP prompts/list result must contain a prompts array")?;
    for prompt in prompts {
        if prompt.get("name").and_then(Value::as_str).filter(|name| !name.is_empty()).is_none() {
...
pub fn call_tool(cwd: &Path, server_name: &str, tool_name: &str, args: Value, timeout_ms: u64) -> Result<Value, String> {
    if tool_name.is_empty() { return Err("toolName is required".into()); }
    if !args.is_object() { return Err("MCP tool arguments must be an object".into()); }
    with_connection(cwd, server_name, |connection| {
        let value = connection.request(cwd, "tools/call", json!({"name": tool_name, "arguments": args}), timeout_ms)?;
        if !value.is_object() { return Err("MCP tools/call result must be an object".into()); }
        Ok(value)
    })
}
...
pub fn read_resource(cwd: &Path, server_name: &str, uri: &str, timeout_ms: u64) -> Result<Value, String> {
    if uri.is_empty() { return Err("uri is required".into()); }
    with_connection(cwd, server_name, |connection| {
        let value = connection.request(cwd, "resources/read", json!({"uri": uri}), timeout_ms)?;
        if !value.is_object() { return Err("MCP resources/read result must be an object".into()); }
        Ok(value)
    })
}
...
pub fn get_prompt(cwd: &Path, server_name: &str, name: &str, args: Value, timeout_ms: u64) -> Result<Value, String> {
    ...
    if !args.is_object() { return Err("MCP prompt arguments must be an object".into()); }
    with_connection(cwd, server_name, |connection| {
        let value = connection.request(cwd, "prompts/get", json!({"name": name, "arguments": args}), timeout_ms)?;
        if !value.is_object() { return Err("MCP prompts/get result must be an object".into()); }
        Ok(value)
    })
}
...
fn connect_parallel(
    ...
    let worker_count = pending.len().min(thread::available_parallelism().map_or(1, usize::from)).min(8);
    ...
    thread::scope(|scope| {
        let handles = (0..worker_count).map(|_| {
            ...
            scope.spawn(move || {
                ...
                loop {
                    let item = queue.lock().map_err(|_| "MCP startup queue lock is poisoned".to_string())?.pop_front();
                    let Some((name, mut connection)) = item else { break };
        ...
        for handle in handles {
            completed.extend(handle.join().map_err(|_| "MCP startup worker panicked".to_string())??);
        }
        Ok(completed)
    })
}
...
pub fn live_tools(cwd: &Path, timeout_ms: u64) -> Result<Vec<(String, Value)>, String> {
    ...
    let pending = {
        ...
        for connection in manager.servers.values_mut() {
            let notifications = match connection.client.as_mut() {
                Some(client) => {
                    if client.is_alive() { client.poll_notifications() } else { continue }
                }
                None => continue,
            };
            match notifications {
                Ok(notifications) => {
                    if let Err(error) = connection.process_notifications(cwd, notifications, timeout_ms) {
        ...
        let names = manager.servers.iter_mut().filter_map(|(name, connection)| {
            let alive = connection.client.as_mut().map(McpClient::is_alive).unwrap_or(false);
            (!alive).then(|| name.clone())
        }).collect::<Vec<_>>();
        names.into_iter().filter_map(|name| manager.servers.remove(&name).map(|connection| (name, connection))).collect::<Vec<_>>()
    };
    ...
    let tools = manager.servers.iter().filter(|(_, connection)| matches!(connection.state, ConnectionState::Ready)).flat_map(|(server, connection)| {
        connection.tools.get("tools").and_then(Value::as_array).into_iter().flatten().cloned().map(move |tool| (server.clone(), tool))
    }).collect::<Vec<_>>();
    if tools.is_empty() && !startup_errors.is_empty() {
        return Err(format!("no MCP server reached ready state: {}", startup_errors.join("; ")));
    }
    Ok(tools)
}
...
pub fn refresh_all(cwd: &Path, timeout_ms: u64) -> Result<Value, String> {
    ...
    let pending = configs.into_iter().map(|(name, config)| (name, ServerConnection::new(config))).collect();
    let results = connect_parallel(pending, cwd, timeout_ms, true)?;
    let key = session_key(cwd);
    let mut managers = managers()?;
    let manager = managers.entry(key).or_default();
    let mut status = serde_json::Map::new();
    let configured_names = results.iter().map(|(name, _, _)| name.clone()).collect::<BTreeSet<_>>();
    manager.servers.retain(|name, _| configured_names.contains(name));
    for (name, connection, result) in results {
...
pub fn reconnect(cwd: &Path, server_name: &str, timeout_ms: u64) -> Result<Value, String> {
    ...
    with_connection(cwd, server_name, |connection| {
        ...
        crate::capability::invalidate();
        Ok(json!({"server": server_name, "state": connection.state.as_str(), "tools": connection.tools.get("tools").and_then(Value::as_array).map_or(0, Vec::len)}))
    })
}
...
pub(crate) fn capability_descriptors(cwd: &Path) -> Vec<CapabilityDescriptor> {
    ...
    let Ok(mut managers) = managers() else {
        return vec![CapabilityDescriptor::new(
            "service/mcp", CapabilityKind::Service, "jeden-core", "MCP manager",
            "Persistent MCP connection manager", FunctionTarget::Service { name: "mcp".into() },
        ).operation("refresh").health(CapabilityHealth::unavailable("MCP manager lock is poisoned"))];
    };
    let manager = managers.entry(key).or_default();
    if let Err(error) = manager.sync_config(cwd) {
        return vec![CapabilityDescriptor::new(
            "service/mcp", CapabilityKind::Service, "jeden-core", "MCP manager",
            "Persistent MCP connection manager", FunctionTarget::Service { name: "mcp".into() },
        ).operation("refresh").health(CapabilityHealth::unavailable(error))];
    }
    let mut out = Vec::new();
    out.push(CapabilityDescriptor::new(
        "service/mcp", CapabilityKind::Service, "jeden-core", "MCP manager",
        "Persistent bounded MCP connection manager", FunctionTarget::Service { name: "mcp".into() },
    ).operation("refresh").operation("status").health(match discovery_error {
        Some(error) => CapabilityHealth { state: crate::capability::HealthState::Degraded, detail: Some(error) },
    ...
    for (server, connection) in &manager.servers {
        ...
        let health = if ready {
            ...
            CapabilityHealth::unavailable(connection.last_error.clone().unwrap_or_else(|| format!("MCP server is {}", connection.state.as_str())))
        };
        out.push(CapabilityDescriptor::new(
            format!("mcp/{server}"), CapabilityKind::Mcp, format!("mcp:{server}"), server.clone(),
            format!("MCP server {server}"), FunctionTarget::McpServer { name: server.clone() },
        ).operation("tools/list").operation("resources/list").operation("prompts/list").health(health.clone()));
        for tool in connection.tools.get("tools").and_then(Value::as_array).into_iter().flatten() {
            let Some(remote_name) = tool.get("name").and_then(Value::as_str) else { continue };
            let native_name = crate::tools::native_mcp_tool_name(server, remote_name);
            let description = tool.get("description").and_then(Value::as_str).map(str::to_string).unwrap_or_else(|| format!("MCP tool {remote_name} from {server}"));
            out.push(CapabilityDescriptor::new(
