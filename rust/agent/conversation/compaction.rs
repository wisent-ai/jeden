...
impl Conversation {
    pub(in crate::agent) fn maybe_auto_compact(&mut self, args: &Args, hooks: &mut RunHooks, reason: &str, add_continue_prompt: bool) -> Result<bool, String> {
        let Some(threshold) = Self::auto_compaction_threshold() else { return Ok(false); };
        ...
        self.recorder.record("auto_compaction", json!({ "reason": reason, "tokens": tokens, "threshold": threshold }))?;
        ...
        if let Err(error) = self.compact(args, &instructions, hooks) {
            self.recorder.record("auto_compaction_error", json!({ "reason": reason, "error": error }))?;
    ...
    pub(crate) fn compact(&mut self, args: &Args, instructions: &str, hooks: &mut RunHooks) -> Result<String, String> {
        ...
        let summary_completion = chat_completion(&router, ask, args.max_tokens.map(|t| t as usize), &[])?;
        if let Some(usage) = &summary_completion.usage {
            let _ = append_usage_event(&args.cwd, &router, usage, usage_cost(&config, &router.model, usage));
        }
        ...
        self.recorder.record("compaction", json!({ "before": before, "summary": summary.clone() }))?;
        self.messages = vec![
    ...
    pub(crate) fn handoff(&mut self, args: &Args, focus: &str, hooks: &mut RunHooks) -> Result<String, String> {
        ...
        let brief_completion = chat_completion(&router, ask, args.max_tokens.map(|t| t as usize), &[])?;
        ...
        self.recorder.record("handoff", json!({ "focus": focus, "brief": brief.clone(), "file": file }))?;
    ...
    pub(in crate::agent::conversation) fn maybe_advisor_review(&mut self, args: &Args, answer: String, hooks: &mut RunHooks) -> Result<String, String> {
        ...
        match chat_completion(&router, ask, args.max_tokens.map(|t| t as usize), &[]) {
            Ok(review_completion) => {
                ...
                self.recorder.record("advisor", json!({ "review": review.clone() })).ok();
                ...
                if let Some(obj) = state.as_object_mut() {
                    ...
                    if let Some(advisor_obj) = advisor.as_object_mut() {
                        advisor_obj.insert("lastReview".into(), json!({ "text": review.clone(), "at": now_stamp() }));
                    }
