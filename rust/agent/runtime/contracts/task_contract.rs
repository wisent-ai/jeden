...
pub(crate) fn section(language: &UiLanguage) -> String {
    let mut text = if language.code() == "pl" {
        "Kontrakt zadania:\nZadanie obejmuje trwałe zachowanie produktu, nie jednorazową naprawę. Nie zastępuj funkcjonalności ręcznym restartem ani skryptem inline. Naprawiaj usterki związane z przydzielonym zadaniem; nie rozszerzaj go na niezwiązane obserwacje. Przed implementacją ustal kryteria ukończenia i właściwe interfejsy na podstawie kontraktu produktu. Każdy punkt wymaga opisu wykonania. Pytanie, czytanie lub plan nie upoważniają do zmian: wyjaśnij wtedy, dlaczego dany punkt nie dotyczy zadania. Odpowiedź nie może się skończyć, dopóki zostaje przydzielona praca, którą możesz wykonać sam: nazwanie następnego kroku nie jest jego wykonaniem. Ograniczenia użytkownika i uprawnień nadal obowiązują.\n".to_string()
    } else {
        "Task contract:\nDeliver durable product behavior, not a one-time repair. Never substitute a manual restart or inline script for a product capability. Fix defects related to the assigned task, not unrelated observations. Establish completion criteria and applicable surfaces from the product contract before implementation. Explain every requirement. A question, reading request or plan does not authorize changes: explain why the corresponding requirements do not apply. An answer may not end while assigned work you can do yourself is left: naming the next step is not delivering it. User scope restrictions and tool permissions still apply.\n".to_string()
    };
    text.push_str(scope_sentence(language));
    text.push_str(if language.code() == "pl" {
        "Nie modyfikuj źródeł, binariów ani zainstalowanych pakietów produktów należących do innych podmiotów, takich jak OMP. Lokalna kopia, fork, zmiana nazwy lub adresu repozytorium nie przenosi własności. Korzystaj z udokumentowanej konfiguracji, API i rozszerzeń utrzymywanych poza cudzymi źródłami; naprawiaj integrację w naszym produkcie. Jeśli wymaganie wymagałoby zmiany OMP, przejdź całkowicie na Jeden. Aktualizacja albo odinstalowanie oficjalnym mechanizmem nie jest łataniem cudzego kodu.\n"
    } else {
