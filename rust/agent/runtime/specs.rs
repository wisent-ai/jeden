...
pub(in crate::agent) fn rust_tool_specs(cwd: &Path) -> Vec<Value> {
    let mut specs = vec![
        ...
        tool_spec("context_recommend", "Ask the context advisor what to read for a task instead of searching: ranked locators from the documentation corpus, Jeden memory, the Transcript Lake archive and the Wisent ground-truth index, plus the state of every source that answered nothing. Sources: docs, ground-truth, memory, transcripts, all.", json!({"query": {"type": "string"}, "limit": {"type": "number"}, "sources": {"type": "string"}, "timeoutMs": {"type": "number"}}), vec!["query"]),
