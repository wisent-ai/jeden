...
pub(in crate::agent) fn rust_tool_specs(cwd: &Path) -> Vec<Value> {
    let mut specs = vec![
        ...
        tool_spec("fetch_url", "Fetch one HTTP(S) URL and return capped text; supports optional line range; runs until the server answers", json!({"url": {"type": "string"}, "maxBytes": {"type": "number"}, "range": {"type": "string"}}), vec!["url"]),
        tool_spec("fetch_readable_url", "Fetch one HTTP(S) URL and return simplified readable text with optional line range; runs until the server answers", json!({"url": {"type": "string"}, "maxBytes": {"type": "number"}, "range": {"type": "string"}}), vec!["url"]),
        ...
        tool_spec("todo", "Track retained session tasks. init/append are additive; start begins work; done requests independent verification and never marks work complete. drop/rm may withdraw only agent-created planning items, never user tasks. Use stable task ids from view.", json!({"op": {"type": "string"}, "list": {"type": "array"}, "phase": {"type": "string"}, "items": {"type": "array"}, "task": {"type": "string"}}), vec!["op"]),
        tool_spec("task_evidence", "Read one checksum-validated tool receipt from a Jeden session in this workspace. Session and event ids come from Jeden's evidence index or a tool result's jedenEvidence field.", json!({"sessionPath": {"type": "string"}, "eventId": {"type": "string"}}), vec!["sessionPath", "eventId"]),
        ...
        tool_spec("context_recommend", "Ask the context advisor what to read for a task instead of searching: ranked locators from every readable file, Jeden memory, the Transcript Lake archive and the Wisent ground-truth index, plus the state of every source that answered nothing. Sources: files, ground-truth, memory, transcripts, all. Every source runs to completion.", json!({"query": {"type": "string"}, "limit": {"type": "number"}, "sources": {"type": "string"}}), vec!["query"]),
        tool_spec("mcp_list_tools", "List tools from a configured stdio MCP server", json!({"server": {"type": "string"}}), vec!["server"]),
        tool_spec("mcp_call_tool", "Call one tool on a configured stdio MCP server; the call runs until the server answers", json!({"server": {"type": "string"}, "tool": {"type": "string"}, "args": {"type": "object"}}), vec!["server", "tool"]),
        tool_spec("mcp_list_resources", "List resources from a configured stdio MCP server", json!({"server": {"type": "string"}}), vec!["server"]),
        tool_spec("mcp_read_resource", "Read one resource from a configured stdio MCP server", json!({"server": {"type": "string"}, "uri": {"type": "string"}}), vec!["server", "uri"]),
        tool_spec("mcp_list_prompts", "List prompts from a configured stdio MCP server", json!({"server": {"type": "string"}}), vec!["server"]),
        tool_spec("mcp_get_prompt", "Get one prompt from a configured stdio MCP server", json!({"server": {"type": "string"}, "name": {"type": "string"}, "args": {"type": "object"}}), vec!["server", "name"]),
    ];
    let mut seen = specs
        .iter()
        .filter_map(|spec| {
