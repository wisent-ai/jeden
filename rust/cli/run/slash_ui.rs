...
mod tests {
    ...
    fn picker_for(cwd: &Path, input: &str, model: Option<&str>) -> PickerSpec {
        match interactive_view(cwd, input, model) {
            Some(Ok(CommandOutcome::Picker(spec))) => spec,
            other => panic!("expected picker for {input:?}, got {other:?}"),
        }
    }

    #[test]
    fn native_ui_parameterless_settings_returns_expected_picker() {
        let settings = picker_for(Path::new("."), "/settings", None);
        assert_eq!(settings.title, "Jeden settings");
        for key in [
            "tools.approvalMode",
    ...
    #[test]
    fn native_ui_direct_arguments_and_emitted_picker_commands_bypass_the_view_adapter() {
        let cwd = Path::new(".");
        for input in [
            "/login provider-from-live-data",
            "/model model-from-live-data",
            "/settings get tools.approvalMode",
        ] {
            assert!(
                interactive_view(cwd, input, Some("route-z")).is_none(),
                "direct command {input:?} must continue to command dispatch"
            );
        }
