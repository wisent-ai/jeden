...
use crate::{dirs_home, omp_agent_dir, read_json, slash};
...
fn push_discovered_command(
    ...
    if valid_file_command_name(&name) && seen.insert(name.clone()) {
        out.push(DiscoveredCommand { name, path, source: source.to_string() });
    }
}
...
fn add_flat_command_dir(out: &mut Vec<DiscoveredCommand>, seen: &mut std::collections::BTreeSet<String>, dir: PathBuf, source: &str) {
    for path in md_files(&dir, false) {
        let Some(name) = path.file_stem().and_then(|s| s.to_str()).map(ToString::to_string) else { continue };
        push_discovered_command(out, seen, name, path, source);
    }
}

fn add_frontmatter_name_dir(out: &mut Vec<DiscoveredCommand>, seen: &mut std::collections::BTreeSet<String>, dir: PathBuf, source: &str) {
    for path in md_files(&dir, false) {
        let name = fs::read_to_string(&path)
            .ok()
            .and_then(|text| frontmatter_field(&text, "name"))
            .or_else(|| path.file_stem().and_then(|s| s.to_str()).map(ToString::to_string));
        let Some(name) = name else { continue };
        push_discovered_command(out, seen, name, path, source);
    }
}

fn add_claude_command_dir(out: &mut Vec<DiscoveredCommand>, seen: &mut std::collections::BTreeSet<String>, dir: PathBuf, source: &str) {
    for path in md_files(&dir, true) {
        let Some(stem) = path.file_stem().and_then(|s| s.to_str()).map(ToString::to_string) else { continue };
        push_discovered_command(out, seen, stem.clone(), path.clone(), source);
        if let Ok(relative) = path.strip_prefix(&dir) {
            let mut parts = relative.iter().filter_map(|part| part.to_str()).collect::<Vec<_>>();
...
fn parse_bool_text(value: &str) -> Option<bool> {
    match value.trim().trim_matches('"').trim_matches('\'').to_ascii_lowercase().as_str() {
...
/// Discover file-based custom slash commands with OMP-like provider precedence.
/// First discovered command name wins across providers.
pub(crate) fn discover_file_commands(cwd: &Path) -> Vec<DiscoveredCommand> {
    let mut out = Vec::new();
    let mut seen = std::collections::BTreeSet::new();
    add_flat_command_dir(&mut out, &mut seen, cwd.join(".jeden/commands"), "Jeden Project");
    add_flat_command_dir(&mut out, &mut seen, cwd.join(".omp/commands"), "OMP Project");
    add_flat_command_dir(&mut out, &mut seen, dirs_home().join(".jeden/commands"), "Jeden User");
    add_flat_command_dir(&mut out, &mut seen, omp_agent_dir().join("commands"), "OMP User");
    ...
    if command_provider_enabled(cwd, "enableClaudeUser", true) {
        add_claude_command_dir(&mut out, &mut seen, dirs_home().join(".claude/commands"), "Claude User");
    }
    if command_provider_enabled(cwd, "enableClaudeProject", true) {
        add_claude_command_dir(&mut out, &mut seen, cwd.join(".claude/commands"), "Claude Project");
    }
    add_frontmatter_name_dir(&mut out, &mut seen, dirs_home().join(".codex/commands"), "Codex User");
    add_frontmatter_name_dir(&mut out, &mut seen, cwd.join(".codex/commands"), "Codex Project");
    if command_provider_enabled(cwd, "enableOpencodeUser", true) {
        add_frontmatter_name_dir(&mut out, &mut seen, dirs_home().join(".config/opencode/commands"), "OpenCode User");
    }
    if command_provider_enabled(cwd, "enableOpencodeProject", true) {
        add_frontmatter_name_dir(&mut out, &mut seen, cwd.join(".opencode/commands"), "OpenCode Project");
    }
    out
}
