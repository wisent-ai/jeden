...
pub(crate) fn mime_type_for_path(path: &Path) -> &'static str {
    match path
        ...
        .as_str()
    {
        "txt" | "md" | "rs" | "js" | "ts" | "tsx" | "json" | "toml" | "yaml" | "yml" | "html"
        | "css" | "csv" => "text/plain",
        "png" => "image/png",
        "jpg" | "jpeg" => "image/jpeg",
        "gif" => "image/gif",
...
pub(crate) fn parse_line_range_token(
    token: &str,
    line_count: usize,
    full_range: &str,
    ...
    let (start, end) = if let Some((a, b)) = trimmed.split_once('+') {
        let start = a
            .parse::<usize>()
            .map_err(|_| format!("invalid range: {full_range}"))?;
        let count = b
            .parse::<usize>()
            .map_err(|_| format!("invalid range: {full_range}"))?;
    ...
    if start == 0 || end < start {
        return Err(format!("invalid range: {full_range}"));
    }
...
pub(crate) fn line_window(
    text: &str,
    range: &str,
) -> Result<(String, usize, usize, Vec<Value>), String> {
    ...
    if lines.is_empty() {
        return Ok((
            ...
            1,
            0,
            vec![json!({"startLine": 1, "endLine": 0})],
        ));
    }
    ...
    for token in range.split(',') {
        ...
        if start_idx < end_idx {
            chunks.push(lines[start_idx..end_idx].join("\n"));
        }
...
pub(crate) fn verify_expected_sha(
    path_label: &str,
    file: &Path,
    expected: &str,
    ...
    if actual != expected {
        return Err(format!(
            "expectedSha256 mismatch for {path_label}: expected {expected}, actual {actual}"
        ));
    }
    Ok(bytes)
}
