...
pub(crate) fn read_any(runtime: &ToolRuntime<'_>, input: &Value) -> Result<Value, String> {
    ...
    for suffix in [".tar.gz:", ".tgz:", ".tar:", ".zip:"] {
        if let Some(index) = lower.find(suffix) {
            ...
            return read_archive(runtime, &routed);
        }
    }
    if let Some(index) = lower.find(".sqlite:").or_else(|| lower.find(".db:")) {
        let extension = if lower[index..].starts_with(".sqlite:") {
            8
        } else {
            4
        };
        let split = index + extension - 1;
        let selector = &path[split + 1..];
        let mut routed = input.clone();
        let object = routed
            .as_object_mut()
            .ok_or("read input must be an object")?;
        object.insert("path".into(), json!(&path[..split]));
        let (table, key) = selector
            .split_once(':')
            .map_or((selector, None), |(table, key)| (table, Some(key)));
        object.insert("table".into(), json!(table));
        if let Some(key) = key {
            object.insert("key".into(), json!(key));
        }
        return read_sqlite(runtime, &routed);
    }
    let plain = path.split(':').next().unwrap_or(&path).to_ascii_lowercase();
    if plain.ends_with(".ipynb")
        || plain.ends_with(".pdf")
        || plain.ends_with(".csv")
        || plain.ends_with(".html")
        || plain.ends_with(".xml")
    {
        return read_document(runtime, input);
    }
...
fn walk_dir(
    ...
    for entry in children {
        ...
        if metadata.is_dir() && depth > 0 {
            walk_dir(runtime, &path, depth - 1, limit, entries)?;
        }
    }
    Ok(())
}
...
pub(crate) fn list_dir(runtime: &ToolRuntime<'_>, input: &Value) -> Result<Value, String> {
    ...
    Ok(
        json!({"ok":true,"path":path,"depth":depth,"limit":limit,"truncated":entries.len()==limit,"entries":entries}),
    )
}
