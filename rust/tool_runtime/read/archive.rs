...
fn zip_entries(bytes: &[u8]) -> Result<Vec<ArchiveEntry>, String> {
    ...
    loop {
        if le_u32(bytes, i).ok() == Some(0x06054b50) {
            eocd = Some(i);
            break;
        }
        if i == 0 {
    ...
    let mut entries = Vec::new();
    for _ in 0..total {
        if le_u32(bytes, offset)? != 0x02014b50 {
...
pub(crate) fn read_archive(runtime: &ToolRuntime<'_>, input: &Value) -> Result<Value, String> {
    ...
    if string_input(input, "entry").is_none() {
        let listed = entries
            .iter()
            .map(|entry| json!({"name": entry.name, "type": entry.kind, "bytes": entry.size}))
            .collect::<Vec<_>>();
    ...
    let entry = entries
        ...
        .ok_or_else(|| format!("archive entry not found: {entry_name}"))?;
    if entry.kind != "file" {
    ...
    if mode == "binary" {
        return Ok(
            json!({"path": path, "entry": entry.name, "mode": mode, "bytes": entry.content.len(), "truncated": entry.content.len() > sliced.len(), "mimeType": mime_type_for_path(Path::new(&entry.name)), "base64": general_purpose::STANDARD.encode(sliced), "sha256": sha256_hex(&entry.content)}),
        );
    }
    if mode == "image" {
        let Some((width, height, mime_type)) = image_dimensions(&entry.content) else {
            return Err(format!(
                ...
                mime_type_for_path(Path::new(&entry.name))
            ));
        };
        return Ok(
            json!({"path": path, "entry": entry.name, "mode": mode, "bytes": entry.content.len(), "truncated": entry.content.len() > sliced.len(), "mimeType": mime_type, "width": width, "height": height, "base64": general_purpose::STANDARD.encode(sliced), "sha256": sha256_hex(&entry.content)}),
        );
    }
    ...
    if mode == "document" {
        out["mimeType"] = json!(mime_type_for_path(Path::new(&entry.name)));
    }
    Ok(out)
}
