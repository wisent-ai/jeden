...
fn parse_visual_body(lines: &[&str], mut index: usize) -> Result<(Vec<String>, usize), String> {
    ...
    while index < lines.len() {
        ...
        let Some(rest) = line.strip_prefix('+') else {
            return Err(format!("patch body line must start with +: {line}"));
        };
...
pub(super) fn parse_visual_edit_patch(patch: &str) -> Result<Vec<VisualPatchSection>, String> {
    ...
    while index < lines.len() - 1 {
        ...
        if line.starts_with('[') && line.ends_with(']') {
            ...
            let Some((path, tag)) = inner.rsplit_once('#') else {
                return Err(format!("invalid file header: {line}"));
            };
            sections.push(VisualPatchSection {
                path: path.to_string(),
        ...
        let Some(current) = sections.last_mut() else {
            return Err("patch hunk appears before file header".into());
        };
        if let Some(rest) = line.strip_prefix("SWAP.BLK ") {
            let line_no = rest
                .strip_suffix(':')
                .ok_or_else(|| format!("unsupported patch line: {line}"))?
                .parse::<usize>()
                .map_err(|_| format!("unsupported patch line: {line}"))?;
            let (content, next) = parse_visual_body(&lines, index + 1)?;
            if content.is_empty() {
                return Err(
                    "SWAP.BLK hunk requires at least one + body line; use DEL.BLK to delete".into(),
                );
            }
            current.ops.push(VisualPatchOp {
                op: "replace_block".into(),
                start_line: None,
                end_line: None,
                line: Some(line_no),
                content,
        ...
        if let Some(rest) = line.strip_prefix("INS.BLK.POST ") {
            ...
            let (content, next) = parse_visual_body(&lines, index + 1)?;
            if content.is_empty() {
                return Err("INS.BLK.POST hunk requires at least one + body line".into());
            }
            current.ops.push(VisualPatchOp {
                op: "insert_block_after".into(),
                start_line: None,
                end_line: None,
                line: Some(line_no),
                content,
        ...
        if let Some(rest) = line.strip_prefix("SWAP ") {
            ...
            if content.is_empty() {
                return Err(
                    "SWAP hunk requires at least one + body line; use DEL to delete".into(),
                );
            }
            current.ops.push(VisualPatchOp {
                op: "replace".into(),
        ...
        if let Some(rest) = line.strip_prefix("INS.PRE ") {
            ...
            let (content, next) = parse_visual_body(&lines, index + 1)?;
            if content.is_empty() {
                return Err("INS hunk requires at least one + body line".into());
            }
            current.ops.push(VisualPatchOp {
                op: "insert_before".into(),
                start_line: None,
                end_line: None,
                line: Some(line_no),
                content,
    ...
    if sections.is_empty() {
        return Err("patch must include at least one file section".into());
    }
    for section in &sections {
        if section.remove && (!section.ops.is_empty() || section.move_to.is_some()) {
            return Err(format!(
                "REM cannot be combined with other hunks: {}",
                section.path
            ));
        }
        if section.ops.is_empty() && !section.remove && section.move_to.is_none() {
            return Err(format!("patch file section has no hunks: {}", section.path));
        }
    }
    Ok(sections)
}
