...
use crate::cli::sessions::{claim_pending_action, complete_pending_action, create_pending_action, discard_pending_action, PendingActionCreate};
...
fn language(name: &str, path: &Path) -> Result<Language, String> {
    let inferred = path.extension().and_then(|value| value.to_str()).unwrap_or("");
...
fn parse<'a>(source: &'a [u8], language: &Language) -> Result<tree_sitter::Tree, String> {
    ...
    parser.parse(source, None).ok_or_else(|| "AST parser returned no tree".to_string())
}

fn source(runtime: &ToolRuntime<'_>, input: &Value) -> Result<(String, PathBuf, Vec<u8>, Language), String> {
    ...
    if !metadata.is_file() || metadata.len() > MAX_AST_BYTES {
        return Err(format!("AST input must be a file no larger than {MAX_AST_BYTES} bytes"));
    }
...
fn selected_capture(query: &Query, input: &Value) -> Result<u32, String> {
    if let Some(name) = string_input(input, "capture") {
        query.capture_index_for_name(&name).ok_or_else(|| format!("query has no @{name} capture"))
...
fn ranges(source: &[u8], tree: &tree_sitter::Tree, query: &Query, capture: u32, limit: usize) -> Vec<(usize, usize, Value)> {
    ...
    while let Some(item) = stream.next() {
        for found in item.captures.iter().filter(|found| found.index == capture) {
            ...
            out.push((node.start_byte(), node.end_byte(), json!({
                "kind": node.kind(),
...
pub(crate) fn ast_search(runtime: &ToolRuntime<'_>, input: &Value) -> Result<Value, String> {
    if runtime.operation.cancellation().is_cancelled() { return Err("AST search cancelled".into()); }
    ...
    let query = Query::new(&language, &query_source).map_err(|error| format!("invalid AST query: {error}"))?;
    let capture = selected_capture(&query, input)?;
    let matches = ranges(&bytes, &tree, &query, capture, limit).into_iter().map(|(_, _, value)| value).collect::<Vec<_>>();
    Ok(json!({"ok": true, "path": label, "query": query_source, "matches": matches, "truncated": matches.len() == limit}))
}
...
fn resolve_pending(runtime: &ToolRuntime<'_>, input: &Value, apply: bool) -> Result<Value, String> {
    ...
    let artifact_dir=runtime.artifact_dir.ok_or("AST pending actions require a recorded session")?;
    if !apply {
        discard_pending_action(artifact_dir,&runtime.operation,&id)?;
        return Ok(json!({"ok":true,"pendingId":id,"discarded":true}));
    }
    if !runtime.allow_write { return Err("applying AST rewrite requires --allow-write".into()); }
    let claim=claim_pending_action(artifact_dir,&runtime.operation,&id)?;
    if claim.kind!="ast_rewrite" { return Err(format!("pending action {} has kind {}, expected ast_rewrite",claim.id,claim.kind)); }
    ...
    if current_sha!=claim.expected_sha256 { return Err(format!("AST rewrite conflict for {}: expected {}, found {}",claim.target,claim.expected_sha256,current_sha)); }
    ...
    Ok(json!({"ok":true,"pendingId":claim.id,"applied":true,"path":claim.target,"sha256":sha256_hex(&claim.payload)}))
}

pub(crate) fn ast_rewrite(runtime: &ToolRuntime<'_>, input: &Value) -> Result<Value, String> {
    match string_input(input, "action").unwrap_or_else(|| "preview".into()).as_str() {
    ...
    if runtime.operation.cancellation().is_cancelled() { return Err("AST rewrite cancelled".into()); }
    let query_source = string_input(input, "query").ok_or("ast_rewrite preview requires query")?;
    let replacement = string_input(input, "replacement").ok_or("ast_rewrite preview requires replacement")?;
    ...
    for pair in selected.windows(2) {
        if pair[0].1 > pair[1].0 { return Err("AST rewrite captures overlap".into()); }
    }
    ...
    let after = std::str::from_utf8(&rewritten).map_err(|_| "AST replacement produced invalid UTF-8")?;
    let diff = simple_diff(&label, before, after);
    let expected_sha256 = sha256_hex(&bytes);
    let artifact_dir=runtime.artifact_dir.ok_or("AST pending actions require a recorded session")?;
    let id=create_pending_action(artifact_dir,&runtime.operation,PendingActionCreate{kind:"ast_rewrite".into(),target:label.clone(),expected_sha256:expected_sha256.clone(),payload:rewritten,preview:diff.clone(),ttl_seconds:PENDING_TTL.as_secs()})?;
    Ok(json!({"ok": true, "preview": true, "pendingId": id, "expiresInMs": PENDING_TTL.as_millis(), "path": label, "expectedSha256": expected_sha256, "matchCount": selected.len(), "diff": diff}))
}
