...
pub(crate) fn todo_tool(runtime: &ToolRuntime<'_>, input: &Value) -> Result<Value, String> {
    let Some(dir) = runtime.artifact_dir else {
        return Err("todo requires an active session".into());
    };
    ...
    if op == "init" {
        ...
        if let Some(phase_name) = string_input(input, "phase")
            .filter(|_| op != "start" && string_input(input, "task").is_none())
        {
            ...
            if !found {
                return Err(format!("unknown phase: {phase_name}"));
            }
        } else {
                ...
                for phase in phases {
                    if let Some(items) = phase["items"].as_array_mut() {
                        for item in items {
                            ...
                            if item.get("text").and_then(Value::as_str) == Some(task.as_str()) {
                                item["status"] = json!(status);
                                found = true;
                            }
                        }
        ...
        if let Some(phase_name) = string_input(input, "phase") {
            if let Some(phases) = state["phases"].as_array_mut() {
                phases.retain(|phase| {
                    phase.get("phase").and_then(Value::as_str) != Some(phase_name.as_str())
                });
            }
            ...
            if let Some(phases) = state["phases"].as_array_mut() {
                for phase in phases {
                    if let Some(items) = phase["items"].as_array_mut() {
                        items.retain(|item| {
                            item.get("text").and_then(Value::as_str) != Some(task.as_str())
                        });
                    }
                }
            }
