...
mod tests {
    use super::super::{ArtifactSink, CancellationToken, ExecutionGrant};
    use super::*;
    use std::fs;
    ...
    impl PtyFixture {
        fn new(name: &str) -> Self {
            ...
            let root = std::env::temp_dir()
                .join(format!("jeden-pty-{name}-{}-{unique}", std::process::id()));
    ...
    fn resize_updates_the_live_pty_and_repeated_resize_reaches_the_shell() {
        if !super::super::SecureRuntime::detect().health().enforced() {
            return;
        }
    ...
    fn resize_rejects_missing_ended_cancelled_and_operation_cancelled_sessions() {
        ...
        let ended_id = ended
            .execute(&ended_context, "printf created")
            .unwrap()
            .session
            .session_id;
        ...
        let teardown_id = torn_down
            .execute(&teardown_context, "printf created")
            .unwrap()
            .session
            .session_id;
        ...
        let live_id = live
            .execute(&live_context, "printf created")
            .unwrap()
            .session
            .session_id;
    ...
    #[test]
    fn degraded_sandbox_rejects_pty_before_canary_file_creation() {
        if super::super::SecureRuntime::detect().health().enforced() {
            return;
        }

        let fixture = PtyFixture::new("degraded-canary");
        let canary = fixture.root.join("pty-canary");
        let grant = ExecutionGrant::trusted_host("pty-test", fixture.root.clone());
        let context = fixture.context().with_execution_grant(grant);
        let input = format!("printf created > {}", canary.display());

        let error = fixture
            .execute(&context, &input)
            .err()
            .expect("degraded sandbox must reject untrusted PTY");

        assert!(
            error.contains("enforced sandbox unavailable"),
            "unexpected PTY rejection: {error}"
        );
        assert!(!canary.exists(), "rejected PTY created its canary file");
    }

    #[test]
    fn expired_grant_rejects_pty_before_canary_file_creation() {
        let fixture = PtyFixture::new("expired-canary");
        let canary = fixture.root.join("expired-pty-canary");
        let mut grant = ExecutionGrant::trusted_host("pty-test", fixture.root.clone());
        grant.expires_at = SystemTime::UNIX_EPOCH;
        let context = fixture.context().with_execution_grant(grant);
        let input = format!("printf created > {}", canary.display());
        ...
        let error = fixture
            .execute(&context, &input)
            .err()
            .expect("expired grant must reject untrusted PTY");

        assert_eq!(error, "execution grant expired");
        assert!(!canary.exists(), "expired PTY created its canary file");
    }
}
