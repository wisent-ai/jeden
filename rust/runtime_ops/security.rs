...
mod tests {
    use super::*;
    #[test]
    fn child_grant_cannot_escalate_or_change_principal() {
        let root = std::env::current_dir().unwrap();
        let mut parent = ExecutionGrant::trusted_host("user", root.clone());
        parent.process.programs = ["sh".into()].into_iter().collect();
        parent.network.hosts = ["example.com".into()].into_iter().collect();
        parent.network.ports = [443].into_iter().collect();
        let mut child = parent.clone();
        child.process.programs = ["sh".into(), "python3".into()].into_iter().collect();
        child.network.hosts = ["example.com".into(), "metadata.google.internal".into()]
            .into_iter()
            .collect();
        child.resource_limits.processes = parent.resource_limits.processes + 100;
        let effective = parent.intersect(&child).unwrap();
        assert_eq!(
            effective.process.programs,
            ["sh".into()].into_iter().collect()
        );
        assert_eq!(
            effective.network.hosts,
            ["example.com".into()].into_iter().collect()
        );
        assert_eq!(
            effective.resource_limits.processes,
            parent.resource_limits.processes
        );
        child.principal.subject = "attacker".into();
        assert_eq!(
            parent.intersect(&child).unwrap_err(),
            GrantError::PrincipalChanged
        );
    }
}
