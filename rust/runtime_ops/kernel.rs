...
mod tests {
    ...
    impl KernelFixture {
        ...
        fn context(&self, grant: ExecutionGrant) -> OperationContext<'static> {
            OperationContext::new(
                CancellationToken::new(),
    ...
    #[test]
    fn degraded_sandbox_rejects_eval_before_canary_file_creation() {
        if super::super::SecureRuntime::detect().health().enforced() {
            return;
        }

        let fixture = KernelFixture::new("degraded-canary");
        let canary = fixture.root.join("eval-canary");
        let code = format!(
            "open({}, 'w').write('created')",
            serde_json::to_string(&canary.to_string_lossy()).unwrap()
        );
        let grant = ExecutionGrant::trusted_host("kernel-test", fixture.root.clone());
        let context = fixture.context(grant);
        ...
        .err()
        .expect("degraded sandbox must reject untrusted eval");

        assert!(
            error.contains("enforced sandbox unavailable"),
            "unexpected eval rejection: {error}"
        );
        assert!(!canary.exists(), "rejected eval created its canary file");
    }

    #[test]
    fn expired_grant_rejects_eval_before_canary_file_creation() {
        let fixture = KernelFixture::new("expired-canary");
        let canary = fixture.root.join("expired-eval-canary");
        let code = format!(
            "open({}, 'w').write('created')",
            serde_json::to_string(&canary.to_string_lossy()).unwrap()
        );
        let mut grant = ExecutionGrant::trusted_host("kernel-test", fixture.root.clone());
        ...
        .err()
        .expect("expired grant must reject untrusted eval");

        assert_eq!(error, "execution grant expired");
        assert!(!canary.exists(), "expired eval created its canary file");
    }
}
