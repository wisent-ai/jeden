...
fn defect_preserves_pause_and_refuses_cancelled_work() {
    ...
    for (target, revision, reason) in [
        (
            id,
            paused["revision"].to_string(),
            "Isolated product journey",
        ),
        (id, added["revision"].to_string(), "Another defect"),
        ("missing", paused["revision"].to_string(), "Unknown target"),
        (id, paused["revision"].to_string(), " "),
    ] {
        let result = home.run(&[
            "todo",
            "defect",
