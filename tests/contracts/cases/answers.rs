...
fn blocked_task_names_its_ask_and_finishes_with_the_operators_answer() {
    ...
    assert_eq!(
        home.value(&["todo", "list", "--json"])["status"],
    ...
    let answered = home.value(&[
        "todo",
        "answer",
        id,
        "--revision",
        &home.state()["revision"].to_string(),
        "--text",
        "The deployment token is WISENT-4471.",
        "--json",
    ]);
    let task = answered["tasks"]
        .as_array()
        .unwrap()
        .iter()
    ...
    assert!(task["operatorRequest"]["ask"]
        .as_str()
        .is_some_and(|ask| !ask.is_empty()));
    assert_eq!(
        task["operatorRequest"]["answer"]["text"],
        "The deployment token is WISENT-4471."
    );
    ...
    assert_eq!(
        fs::read(home.workspace().join("token.txt")).unwrap(),
        b"WISENT-4471"
    );
