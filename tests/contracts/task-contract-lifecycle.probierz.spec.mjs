...
const root = join(process.env.PROBIERZ_ARTIFACTS || join(repository, "target/contract-runs"), `runner-${randomUUID()}`);
await mkdir(root, { recursive: true });
const tracePath = join(root, "trace.json");
const trace = { status: "failed", sourceRevision: null, commands: [], binaries: [], completedAt: null };
...
try {
  ...
  const build = await command(["cargo", "test", "--locked", "--release", "--test", "contracts", "--no-run", "--message-format=json"]);
  succeeded(build);
  const artifacts = build.stdout.split("\n").filter(Boolean).map((line) => JSON.parse(line))
    .filter((event) => event.reason === "compiler-artifact" && event.executable);
  const tests = artifacts.filter((event) => event.target.name === "contracts" && event.profile.test);
  assert.equal(tests.length, 1, "Cargo must return exactly the current contract test executable");
