...
struct Home { root: PathBuf, sequence: Cell<usize> }
impl Home {
    fn new(story: &str) -> Self {
        let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("target/contract-runs")
        ...
        fs::write(root.join("source.json"), serde_json::to_vec_pretty(&json!({
        ...
        Self { root, sequence: Cell::new(usize::default()) }
    }
    fn workspace(&self) -> PathBuf { self.root.join("workspace") }
    fn command(&self) -> Command {
        let mut command = Command::new(env!("CARGO_BIN_EXE_jeden"));
        command.current_dir(self.workspace()).env("HOME", self.root.join("home"))
            .env("JEDEN_SESSION_ROOT", self.root.join("sessions")).env_remove("JEDEN_LANGUAGE");
        command
    }
    fn record(&self, args: &[&str], output: &Output) {
        let number = self.sequence.get() + usize::from(true);
        self.sequence.set(number);
        fs::write(self.root.join("evidence").join(format!("{number}.json")),
            serde_json::to_vec_pretty(&json!({"argv": args, "exitCode": output.status.code(),
                "stdout": String::from_utf8_lossy(&output.stdout),
                "stderr": String::from_utf8_lossy(&output.stderr)})).unwrap()).unwrap();
    }
    ...
    fn ok(&self, args: &[&str]) -> Output {
        ...
        assert!(output.status.success(), "{}: {}", args.join(" "), String::from_utf8_lossy(&output.stderr));
        output
    }
    fn value(&self, args: &[&str]) -> Value { serde_json::from_slice(&self.ok(args).stdout).unwrap() }
    fn session(&self) -> PathBuf {
        let mode: Value = serde_json::from_slice(&fs::read(self.workspace().join(".jeden/mode-state.json")).unwrap()).unwrap();
    ...
    fn control(&self, id: &str, action: &str) -> Value {
        self.value(&["todo", action, id, "--revision", &self.state()["revision"].to_string(),
    ...
    fn passed(&self) {
        fs::write(self.root.join("result.json"), serde_json::to_vec(&json!({"passed":true})).unwrap()).unwrap();
    }
}
...
fn operator_contracts_are_written_read_and_reset_through_the_cli() {
    ...
    home.ok(&["config", "set", "contracts.communication", "Answer in Polish using three plain sentences."]);
    home.ok(&["config", "set", "contracts.functionality", "Finish the requested behavior before answering."]);
    assert_eq!(home.config()["contracts"]["communication"], "Answer in Polish using three plain sentences.");
    assert_eq!(home.config()["contracts"]["functionality"], "Finish the requested behavior before answering.");
    let read = home.ok(&["config", "get", "contracts.functionality"]);
    assert_eq!(String::from_utf8(read.stdout).unwrap().trim(), home.config()["contracts"]["functionality"]);
    ...
    assert_eq!(String::from_utf8(refused.stderr).unwrap().trim(), "Error: unknown config key: contracts.style");
    home.passed();
}
...
fn operator_contracts_are_served_through_rpc_for_jeden_desktop() {
    ...
    assert_eq!(home.config()["contracts"]["communication"], "Short sentences.");
    assert_eq!(home.config()["contracts"]["functionality"], "Finish the task in full.");
    let refused = frames.iter().find(|frame| frame["id"] == "refuse").unwrap();
    assert_eq!(refused["error"]["code"], "invalid_params");
    assert_eq!(refused["error"]["message"], "functionality must be a string");
    assert_eq!(String::from_utf8(home.ok(&["config", "get", "contracts.functionality"]).stdout).unwrap().trim(), "Finish the task in full.");
    home.passed();
}
...
fn contracts_install_preserves_surrounding_rules_and_replaces_stale_contract() {
    ...
    assert!(!home.run(&["contracts", "status", "--file", file]).status.success());
    ...
    home.ok(&["config", "set", "contracts.communication", "Answer in three sentences."]);
    assert!(!home.run(&["contracts", "status", "--file", file]).status.success());
    home.ok(&["contracts", "install", "--file", file]);
    ...
    assert!(!refused.status.success());
    assert_eq!(String::from_utf8(refused.stderr).unwrap().trim(), "Error: --file requires a path");
    home.passed();
}
...
fn retained_requests_survive_new_questions_pause_and_session_recovery() {
    ...
    let first = home.value(&["todo", "add", "Create alpha.txt containing exactly ALPHA.", "--json"]);
    ...
    let stale = home.run(&["todo", "resume", request, "--revision", &first["revision"].to_string(), "--reason", "Stale control"]);
    assert!(!stale.status.success());
    assert_eq!(home.state(), before);
    let source = home.session();
    let resumed = home.run(&["resume", source.to_str().unwrap()]);
    assert!(!resumed.status.success());
    assert_eq!(String::from_utf8(resumed.stderr).unwrap().trim(), "Error: Retained work is paused; resume its tasks before continuing.");
    assert_ne!(home.session(), source);
    assert_eq!(home.state()["requests"], before["requests"]);
    let added = home.value(&["todo", "add", "Explain the previous request without cancelling it.", "--json"]);
    ...
    let denied = home.run(&["todo", "resume", request, "--revision", &final_state["revision"].to_string(), "--reason", "Cannot revive cancelled work"]);
    assert!(!denied.status.success());
...
fn task_contract_delivery_observes_real_files_and_retains_results_on_resume() {
    ...
    let output = home.run(&["run", prompt, "--allow-write", "--max-steps", "24", "--json"]);
    if !output.status.success() {
        assert!(home.state()["requests"].as_array().unwrap().iter().any(|request| request["prompt"] == prompt));
        assert!(!home.workspace().join("alpha.txt").exists() || home.state()["requests"][0]["coverageVerified"] == false);
        panic!("Real model completion failed; not passed: {}", String::from_utf8_lossy(&output.stderr));
    }
    assert_eq!(fs::read(home.workspace().join("alpha.txt")).unwrap(), b"ALPHA");
    assert_eq!(fs::read(home.workspace().join("beta.txt")).unwrap(), b"BETA");
    ...
    for task in home.state()["tasks"].as_array().unwrap() {
        ...
        for reference in task["verification"]["evidence"].as_array().unwrap() {
            ...
            assert!(events.lines().map(|line| serde_json::from_str::<Value>(line).unwrap())
                .any(|event| event["eventId"] == reference["eventId"]));
        }
    }
    let before = home.state()["requests"].clone();
    let modification = fs::metadata(home.workspace().join("alpha.txt")).unwrap().modified().unwrap();
