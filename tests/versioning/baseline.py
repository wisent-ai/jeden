...
Usage:
    python3 tests/versioning/baseline.py             # rewrite released-surface.json
    python3 tests/versioning/baseline.py --stdout    # write nothing, print the document
"""
...
def main(argv: list) -> int:
    root = pathlib.Path(__file__).resolve().parents[TWO]
    if "--marker" in argv:
        print(best_marker())
        return int(False)
    document = build(root)
